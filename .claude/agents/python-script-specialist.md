...
- Workspace root `<root>` = the repo root (exposed to Bash commands as `$PROJECT_ROOT`, set in
  `.claude/settings.local.json`); venv `<root>/.venv`. Use absolute binary paths; never `cd` or
  resolve paths dynamically (hooks block it).
