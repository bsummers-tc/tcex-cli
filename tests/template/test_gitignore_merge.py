...
# ==================================================================
# .gitignore is now template-owned (listed in _app_common template_files)
# ==================================================================
class TestGitignoreTemplateOwned:
    """``gitignore`` is in ``_app_common.template_files`` → ``.gitignore`` is forced.

    Because the parent ``template.yaml`` lists ``gitignore`` (renamed on disk to
    ``.gitignore``), the file is template-owned: a locally-modified project
    ``.gitignore`` is force-overwritten back to the template content by ``update``,
    both on the default (interactive) path and under ``--no-prompt`` — it is never
    treated as a user file to preserve or prompt for.
    """

    @staticmethod
    def _init(template_cli, project_dir, monkeypatch) -> None:
        """Init the project so it has a manifest + the template .gitignore."""
        monkeypatch.chdir(project_dir)
        with patch.object(template_cli, 'ensure_cache', return_value=template_cli._cache_dir('v2')):
            template_cli.update('v2', 'basic', 'playbook', force=True)

    def test_collect_keys_includes_gitignore(self, template_cli):
        """_collect_template_file_keys for playbook/basic now contains '.gitignore'."""
        cache_dir = template_cli._cache_dir('v2')
        keys = template_cli._collect_template_file_keys(cache_dir, 'basic', 'playbook')

        assert '.gitignore' in keys

    def test_locally_modified_gitignore_force_overwritten_default(
        self, template_cli, project_dir, monkeypatch
    ):
        """A locally-edited .gitignore is restored to template content with no prompt."""
        self._init(template_cli, project_dir, monkeypatch)

        # sanity: the initialized .gitignore is the 1508-byte template content
        project_gitignore = project_dir / '.gitignore'
        assert _sha256_file(project_gitignore) == GITIGNORE_CONTENT_SHA

        # user edits .gitignore locally
        project_gitignore.write_text('# user gitignore edits\n')
        assert _sha256_file(project_gitignore) != GITIGNORE_CONTENT_SHA
        ...
        # template-owned .gitignore overwritten back to the 1508-byte template content
        assert _sha256_file(project_gitignore) == GITIGNORE_CONTENT_SHA
        # and the user was never prompted for it
        prompted_files = [call.args[0] for call in mock_prompt.call_args_list]
        assert not any('gitignore' in f for f in prompted_files)

    def test_locally_modified_gitignore_force_overwritten_no_prompt(
        self, template_cli, project_dir, monkeypatch
    ):
        """Under --no-prompt, a locally-edited .gitignore is force-overwritten, not preserved."""
        self._init(template_cli, project_dir, monkeypatch)

        project_gitignore = project_dir / '.gitignore'
        project_gitignore.write_text('# user gitignore edits\n')

        # simulate the template moving on so the file is a clear update candidate
        manifest = json.loads((project_dir / 'manifest.json').read_text())
        manifest['.gitignore']['last_commit'] = 'old_commit_before_update'
        ...
        # template-owned .gitignore overwritten (not preserved as a user file)
        assert _sha256_file(project_gitignore) == GITIGNORE_CONTENT_SHA
